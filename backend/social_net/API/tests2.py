...
def helper_generate_comment_json(cls, author, content):
    ...
    @classmethod
    def helper_generate_post_json(cls, author:dict, date:str, id:str=None,
        ...
        NOTE: id, src, origin, and comments are now optional, but should only
        ever be omitted if we are generating json to send in a post request
        where we want the server to handle generating an id for us, and thus,
        cannot possibly know what id will be used in advance (and thus, src,
        origin, and comments will also be unknown) tests should verify that
        jsons generated by this have all the necessary keys as outlined in the
        spec. TODO.
        ...
        if id and not origin:
            origin = id
        if origin and not src:
            src = origin
        if id and not comments:
        ...
        # XXX NOTE: The bottom 4 attributes should never be missing unless we
        # are using this endpoint: https://github.com/Sean-Meyers/CMPUT404-project-socialdistribution/wiki/Objects#:~:text=POST%20%5Blocal%5D%20create%20a%20new%20post%20but%20generate%20a%20new%20id
        # If we are using that endpoint, the serializer is generating the id,
        # and thus, we cannot generate any of these properties ourself until we
        # get the response from that endpoint's POST, which will hopefully
        # contain teh newly generated id at least. Now that this method allows
        # these properties to be blank, we MUST verify that our response and
        # expected json data contains all of these keys. TODO.
        if id:
            post_json["id"] = id
        if src:
            post_json["source"] = src
        if origin:
            post_json["origin"] = origin
        if comments:
    ...
    def helper_populate_posts(cls, uids, authors, num=20):
        ...
        for i in range(num):
            ...
            models.PostsModel.objects.create(type='post', title=title, id=pid,
                                             ...
                                             published=published)
            jsons.append(cls.helper_generate_post_json(author, published.isoformat(), pid,
                                                     title, source, origin,
    ...
    def test_GET_public_post(self):
        ...
        json_data = response.content#json()
    ...
    def test_PUT(self):
        ...
        new_post_json = self.helper_generate_post_json(self.author_jsons[0],
                                                                       now, pid)
        response = self.client.put(relative_url, new_post_json,
                                                             'application/json')
        json_data = self.client.get(relative_url).content#json()
    ...
    # FIXME: Currently assumes that the author is in the JSON data to be sent to
    # the server in the POST request. This causes "TypeError: create() got
    # multiple values for keyword argument 'author'" because the PostsView for
    # some reason creates the post in the db using the author_id from the url,
    # thus it gets the author id from the post as well from the url, causing
    # that error. I think this is more a bug with the View than with my test
    # (because post json data is supposed to have author json data embedded,
    # which is supposed to have an author id, though maybe this only matters for
    # responding to get requests?), but I am not entirely sure.
    def test_POST_new_post(self):
        ...
        now = datetime.datetime.now().isoformat()
        new_post_json = self.helper_generate_post_json(self.author_jsons[0], now)
